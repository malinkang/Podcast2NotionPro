...
def insert_episode(episodes, d):
    ...
    for index, result in enumerate(episodes):
        ...
        date = None
        page_id = None
        result = check_eposide(eid)

        if result:
            date = utils.get_property_value(result.get("properties").get("日期"))
            page_id = result.get("id")
        if date is None:
            date = pendulum.now(tz="Asia/Shanghai").int_timestamp
        episode = {}
        ...
        if episode.get("date"):
            notion_helper.get_date_relation(
                ...
                date=pendulum.from_timestamp(episode.get("date"), tz="Asia/Shanghai"),
            )
        print(
...
if __name__ == "__main__":
    ...
    result = check_eposide("66878ecf0bc5d7cc700115d0")
    print(utils.get_property_value(result.get("properties").get("日期")))
    # refresh_token()
    # # d = insert_podcast()
    # episodes = get_history()
    # with open("episodes.json","w") as f:
    #     f.write(json.dumps(episodes,indent=4,ensure_ascii=False))
